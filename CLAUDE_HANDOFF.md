...
- 유사문제 생성기는 28문항이 연결되어 있으며 교사용 검토 전용 잠금 상태다. 5단계 난이도에서 문항별 1,000회, 총 140,000개 변형을 독립 검산했다.
- 4번은 두 자리 수끼리의 곱으로 제한하지 않고 한 자리 수와 세 자리 수의 곱도 포함하도록 바로잡았다.
...
- 18번은 색종이 자체가 아니라 접고 자르는 과정을 두 번 반복한다는 지문 이해가 핵심이다. 답 40과 필기 풀이 `18×2+4`는 연결했으며, 같은 구조의 정확한 펼침 그림 생성기는 보류한다.
...
- 20번 답 24와 필기 풀이 `6×4`는 연결했으며, 정육면체 절단의 같은 구조 그림 생성기는 보류한다.
- 남은 생성기 작업은 18번과 20번의 원본 구조 기반 그림 모델 두 개뿐이다. 답안 미연결이나 미검수 상태로 표현하지 않는다.
