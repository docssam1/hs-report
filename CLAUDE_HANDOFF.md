...
- 직전 배포는 파이널 2회 상세 풀이 23문항의 `4e319ff70b41f36b38464d1a7d8288502999aa7e`다. main 푸시·Pages built·실서비스 파일 일치 및 실제 프런트 + 가상 응답으로 누적/정답률/읽기 실패 경계 검사를 마쳤다. 이후 30문항 변경의 실제 배포 상태는 현재 Git와 Pages로 별도 확인한다. 실제 학생 기록은 수정하지 않았다.
...
- 후속 작성은 멈추지 않고 진행한다. Final2 30개 분류 후보는 작성 완료/독립 검수 대기이며, 기존 작성 담당자가 Final3 원본 1–15번 상세 풀이 작성으로 이동했다. 문항별 구체 설명 제목과 여러 회차에 재사용할 세부유형은 분리하고, 적절한 소영역이 없다고 가까운 이름에 억지로 넣지 않는다. 이 후속 후보는 아직 공개 승인된 자료가 아니다.
