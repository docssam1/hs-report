...
## 현재 진행 — 2026-09-09 파이널 4회 상세 풀이

- 파이널 4회 원본 6쪽을 1–10, 11–20, 21–30으로 나누어 독립 재계산했다. 첫 공개 집합은 원본과 답이 확인된 1,2,3,4,5,6,7,8,10번의 상세 풀이 9문항이다. 9번은 260알을 정하는 데 필요한 ‘처음 두 약병의 양이 같다’는 조건이 현재 원본에서 보이지 않아 잠갔다.
- 첫 공개 집합은 `final4-detailed-data.js`, 검수 기록은 `qa/final4-detailed-review.json`이다. 원문에 없던 4번의 중간 음수 금지 조건을 제거했고, 3번은 12시간에 직각 22번, 10번은 12자리 곱의 8이 11개임을 정확한 식으로 보강했다. 화면 계약은 상세9/대기21이며 390px와 외부 쓰기0을 검사한다.
- 기존 짧은 답안의 확정 오류도 바로잡았다. 17번은 열차 절반 150m가 이미 들어간 상태이므로 터널 **450m**이고, 24번은 전체 작업시간을 나누면 예정10일보다 **14일** 늦다. 26번은 서로 다른 다섯 카드를 전수 계산해 `241×83=20003`(차3)이다.
- 원본 자체 확인이 필요한 문항은 완료로 표시하지 않는다. 11번은 57을 얻는 누락 수치, 21번은 세 장의 합100(최대27), 27번은 연속 자연수100개의 합84500(첫 수795.5)이 서로 맞지 않는다. 12번 `□2회`, 16번 `□00m`도 가려진 첫 숫자를 별도 근거로 확인해야 한다. 이 문항들은 상세 풀이를 잠근다.
- 다음 순서: 원본과 답이 독립 확인된 13,14,15,18,19,20,22,23,25,28,29,30번과 전수 검산된 26번을 상세 풀이로 작성한다. 17·24번은 수정된 canonical 답에 결합한 뒤 재검수한다. 잠긴 문항은 강의 등 별도 근거가 확인되기 전 추측해서 채우지 않는다.
