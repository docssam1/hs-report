/* GFIELD 관리자 모의고사 결과 v2: 중급/활용 분리, 1·2·3차 개별 삭제, 미리보기 */
(function(){
  ...
  function rowsFor(student,set,r){
    return(MK_ROWS||[]).map(x=>({x,p:parseRoundKey(x.round)})).filter(o=>o.p&&o.p.set===set&&o.x.student===student&&(!r||o.p.round===String(r))).sort((a,b)=>+a.p.round-+b.p.round||a.p.slot-b.p.slot);
  }
  ...
  renderMock=function(){
    ...
    let panel=`<div style="border:1px solid #dce4ef;border-radius:14px;padding:14px;margin:4px 0 16px;background:linear-gradient(135deg,#f8fbff,#fff)">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <span style="margin-left:auto;font-size:11.5px;color:#6b7280">2차만 삭제하면 1·3차는 그대로 유지됩니다.</span>
      </div>
      ...
      list.forEach((o,i)=>{
        ...
        panel+=`<tr><td>${esc(roundTitle(window.mkSet,r))}</td><td><b>${p.slot}차</b></td><td><b>${score}</b></td><td>${wrong}</td><td>${esc(x.source||'-')}</td><td>${esc(at)}</td><td>
          <div style="display:flex;gap:5px;justify-content:center;flex-wrap:wrap">
            ...
            <button class="btn del sm" onclick="deleteMockAttemptV2('${esc(student)}','${window.mkSet}','${r}',${p.slot})">${p.slot}차 삭제</button>
    ...
    if(hint)hint.textContent='중급·활용 모의고사 결과를 분리해 확인합니다. 각 회차의 1·2·3차 기록을 개별 삭제하거나 저장되지 않는 관리자 미리보기로 들어갈 수 있습니다.';
  };

  window.setMockSetV2=function(set){window.mkSet=set==='hw'?'hw':'mid';renderMock()};

  async function del(student,set,r,slot){
    const key=rawKey(set,r,slot),u=MK_URL+'/rest/v1/mock_results?student=eq.'+encodeURIComponent(student)+'&round=eq.'+encodeURIComponent(key);
    const res=await fetch(u,{method:'DELETE',headers:{apikey:MK_KEY,Authorization:'Bearer '+MK_KEY,Prefer:'return=minimal'}});
  ...
  window.deleteMockAttemptV2=async function(student,set,r,slot){
    if(!confirm(`${student} 학생의 ${roundTitle(set,r)} ${slot}차 기록만 삭제할까요?\n다른 차수는 유지됩니다.`))return;
    try{await del(student,set,r,slot);toast(`${slot}차 기록을 삭제했습니다.`);renderMock()}catch(e){toast('기록 삭제에 실패했습니다.')}
  };
  window.deleteMockRoundV2=async function(student,set,r){
    if(!confirm(`${student} 학생의 ${roundTitle(set,r)} 1~3차 기록을 모두 초기화할까요?`))return;
    try{for(let slot=1;slot<=3;slot++)await del(student,set,r,slot);toast('회차 전체를 초기화했습니다.');renderMock()}catch(e){toast('전체 초기화에 실패했습니다.')}
  };
