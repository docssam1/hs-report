(function(root){
  ...
  var SCRIPTED_TUTORS={
    1:{title:'흩어진 샤프심 수',page:'001.jpg',view:'45 300 530 620',summary:'전체 50개에서 그림 속 샤프심과 책상 위 2개를 차례로 빼어 책상 밑의 수를 찾습니다.',answer:'37개',answerLabel:'책상 밑으로 굴러 들어간 샤프심',steps:[
      {title:'그림 속 샤프심부터 세어 봐요',speech:'샤프심은 양 끝이 한 쌍입니다. 그림에서 끝이 22개라면 샤프심은 몇 개일까요?',deep:'한 샤프심마다 끝이 2개이므로 22÷2=11개입니다. 겹친 곳에서도 한 줄의 양 끝을 이어 세면 빠뜨리지 않아요.',visual:'<span>끝 22개</span><i>÷2</i><span><b>샤프심 11개</b></span>'},
      {title:'이미 보이는 수를 합쳐요',speech:'바닥의 11개와 책상 위의 2개를 합치면 이미 위치를 아는 샤프심은 13개입니다.',deep:'책상 밑으로 들어간 것만 남겨야 하므로 바닥과 책상 위를 따로 빼거나 먼저 합쳐도 같습니다.',visual:'<span>바닥 11</span><i>+</i><span>책상 위 2</span><i>=</i><span><b>13</b></span>'},
      {title:'전체에서 보이는 수를 빼요',speech:'처음 50개에서 위치를 아는 13개를 빼면 책상 밑으로 들어간 수가 남습니다.',deep:'50−11−2와 50−(11+2)는 같은 계산입니다.',visual:'<span>전체 50</span><i>−</i><span>보이는 13</span>'},
      {title:'남은 수를 답해요',speech:'50−13=37이므로 책상 밑으로 굴러 들어간 샤프심은 37개입니다.',visual:'<span>50−13</span><i>=</i><span><b>37개</b></span>'}
    ]},
    2:{title:'거울 시계와 지난 시간',page:'001.jpg',view:'45 900 530 430',summary:'거울에 비친 시각을 실제 시각으로 되돌린 뒤 오전 9시까지 지난 시간을 구합니다.',answer:'95분',answerLabel:'다시 잔 시간',steps:[
      {title:'거울 속 시각을 그대로 읽지 않아요',speech:'그림은 실제 시계가 아니라 거울에 비친 모습입니다. 긴바늘과 짧은바늘을 좌우로 되돌려 볼까요?',deep:'거울 시계는 12시를 기준으로 좌우가 바뀝니다. 거울 속 4시 35분을 실제 시각으로 바꾸어야 해요.',visual:'<span>거울 속<br><b>4시 35분</b></span><i>좌우 되돌리기</i><span>실제 시각</span>'},
      {title:'실제 시각을 찾아요',speech:'12시에서 4시 35분을 거꾸로 세면 실제 시각은 오전 7시 25분입니다.',deep:'11시 60분−4시 35분=7시 25분으로 계산해도 됩니다.',visual:'<span>11시 60분</span><i>−</i><span>4시 35분</span><i>=</i><span><b>7시 25분</b></span>'},
      {title:'9시까지 두 구간으로 나눠요',speech:'7시 25분부터 8시 25분까지 1시간, 다시 9시까지 35분입니다.',deep:'1시간은 60분입니다. 아직 두 시간을 합친 최종 분 수는 마지막 단계에서 확인해요.',visual:'<span>1시간</span><i>+</i><span>35분</span>'},
      {title:'분으로 바꾸어 더해요',speech:'60+35=95이므로 다시 잠든 시간은 95분입니다.',visual:'<span>60</span><i>+</i><span>35</span><i>=</i><span><b>95분</b></span>'}
    ]},
    3:{title:'크고 작은 정사각형 세기',page:'001.jpg',view:'600 300 550 450',summary:'6번째 계단형 배열을 완성하고 한 변의 길이별로 정사각형을 나누어 셉니다.',answer:'67개',answerLabel:'크고 작은 정사각형',steps:[
      {title:'6번째 모양의 작은 칸부터 세어요',speech:'아래층부터 11, 9, 7, 5, 3, 1칸입니다. 모두 몇 칸인지 더해 볼까요?',deep:'1+3+5+7+9+11=36이므로 한 변이 한 칸인 정사각형은 36개입니다.',visual:'<span>1+3+5+7+9+11</span><i>=</i><span><b>36</b></span>'},
      {title:'두 칸짜리 정사각형을 찾아요',speech:'가로와 세로가 두 칸인 정사각형은 아래에서부터 층별로 세면 모두 20개입니다.',deep:'모서리 네 곳의 성냥개비가 모두 이어진 것만 셉니다. 비스듬한 모양은 정사각형으로 세지 않아요.',visual:'<span>한 변 2칸</span><i>→</i><span><b>20개</b></span>'},
      {title:'더 큰 정사각형도 끝까지 세어요',speech:'한 변이 세 칸인 것은 9개, 네 칸인 것은 2개이고 그보다 큰 것은 만들 수 없습니다.',deep:'크기별로 따로 표시하면 같은 정사각형을 두 번 세거나 큰 정사각형을 빠뜨리지 않습니다.',visual:'<span>3칸<br><b>9개</b></span><span>4칸<br><b>2개</b></span>'},
      {title:'크기별 개수를 모두 더해요',speech:'36+20+9+2=67이므로 크고 작은 정사각형은 모두 67개입니다.',visual:'<span>36+20+9+2</span><i>=</i><span><b>67개</b></span>'}
    ]},
    4:{title:'별 배열의 개수',page:'001.jpg',view:'600 900 550 500',summary:'별이 가득 찬 직사각형의 전체 자리에서 비어 있는 다섯 부분을 뺍니다.',answer:'102개',answerLabel:'그림의 별',steps:[
      {title:'바깥 직사각형의 가로와 세로를 세어요',speech:'별이 빈틈없이 놓였다고 생각하면 가로 10개, 세로 12개인 직사각형입니다.',deep:'비어 있는 곳 때문에 줄이 끊겨 보여도 바깥 줄을 따라 가로와 세로의 자리 수를 먼저 정합니다.',visual:'<span>가로 10</span><i>×</i><span>세로 12</span>'},
      {title:'전부 채웠을 때의 수를 구해요',speech:'가로 10개씩 12줄이면 전체 자리는 10×12=120개입니다.',deep:'지금 보이는 별을 하나씩 세기보다 완성된 직사각형에서 빈 곳을 빼는 편이 빠르고 정확합니다.',visual:'<span>10×12</span><i>=</i><span><b>120자리</b></span>'},
      {title:'비어 있는 다섯 부분을 세어요',speech:'빈 자리는 부분별로 4개, 6개, 4개, 2개, 2개입니다. 모두 18자리예요.',deep:'서로 겹치지 않는 빈 부분만 묶어 4+6+4+2+2로 셉니다.',visual:'<span>4+6+4+2+2</span><i>=</i><span><b>18자리</b></span>'},
      {title:'전체 자리에서 빈 자리를 빼요',speech:'120−18=102이므로 그림의 별은 모두 102개입니다.',visual:'<span>120−18</span><i>=</i><span><b>102개</b></span>'}
    ]},
    5:{title:'벌집 모양 길 찾기',page:'002.jpg',view:'45 115 530 500',summary:'한 번 지난 칸을 다시 지나지 않는다는 조건으로 첫걸음을 두 갈래로 나누어 모든 길을 셉니다.',answer:'11가지',answerLabel:'X에서 Y까지 가는 방법',steps:[
      {title:'첫걸음을 두 갈래로 나눠요',speech:'한 번 지난 칸은 다시 지나지 않고, 변을 함께 쓰는 칸으로만 움직입니다. X에서 첫걸음은 가운데 칸 또는 오른쪽 아래 칸입니다.',deep:'꼭짓점만 닿은 칸으로는 건너가지 않습니다. 첫걸음을 기준으로 나누면 같은 길을 두 번 세지 않고 빠짐없이 찾을 수 있어요.',visual:'<span>X → 가운데</span><span>X → 오른쪽 아래</span>'},
      {title:'가운데로 먼저 가는 길을 세어요',speech:'X에서 가운데로 먼저 가면 Y로 바로 가거나, 왼쪽 아래를 거치거나, 아래와 왼쪽 아래를 거치거나, 오른쪽 아래·아래·왼쪽 아래를 거치는 4가지입니다.',deep:'가운데를 지난 뒤에는 가운데로 되돌아올 수 없습니다. 남은 칸을 이어 Y에 도착하는 길을 짧은 것부터 적으면 4가지가 모두 보입니다.',visual:'<span>가운데→Y</span><span>가운데→왼쪽 아래→Y</span><span>가운데→아래→왼쪽 아래→Y</span><span>가운데→오른쪽 아래→아래→왼쪽 아래→Y</span>'},
      {title:'오른쪽 아래로 먼저 가는 길을 세어요',speech:'오른쪽 아래에서 가운데로 가는 갈래는 3가지이고, 아래로 먼저 가는 갈래는 4가지입니다. 따라서 이쪽은 7가지입니다.',deep:'가운데로 간 뒤에는 Y, 왼쪽 아래→Y, 아래→왼쪽 아래→Y의 3가지입니다. 아래로 간 뒤에는 왼쪽 아래→Y, 왼쪽 아래→가운데→Y, 가운데→Y, 가운데→왼쪽 아래→Y의 4가지입니다.',visual:'<span>가운데 쪽<br><b>3가지</b></span><i>+</i><span>아래쪽<br><b>4가지</b></span><i>=</i><span><b>7가지</b></span>'},
      {title:'두 갈래를 합쳐요',speech:'가운데로 먼저 가는 4가지와 오른쪽 아래로 먼저 가는 7가지를 더하면 모두 11가지입니다.',visual:'<span>4가지</span><i>+</i><span>7가지</span><i>=</i><span><b>11가지</b></span>'}
    ]},
    7:{title:'숫자를 바꿔 만든 마지막 쪽',page:'002.jpg',view:'600 115 550 430',summary:'183의 숫자로 만든 뒤쪽 쪽수 중 책의 범위에 드는 끝 쪽을 찾고 처음과 끝을 함께 셉니다.',answer:'136페이지',answerLabel:'뜯어낸 페이지 수',steps:[
      {title:'1·8·3으로 만들 수를 적어요',speech:'183의 세 숫자를 한 번씩 써서 183보다 큰 세 자리 수를 작은 것부터 찾아봅시다.',deep:'318, 381, 813, 831이 뒤쪽 후보입니다. 같은 숫자를 두 번 쓰거나 빠뜨리면 안 됩니다.',visual:'<span>318</span><span>381</span><span>813</span><span>831</span>'},
      {title:'책의 전체 쪽수로 후보를 줄여요',speech:'책은 150페이지이므로 381 이상은 책 안에 없습니다. 마지막 쪽은 318쪽입니다.',deep:'처음 쪽 183보다 뒤이고 책 안에 있는 후보를 두 조건으로 함께 걸러 냅니다.',visual:'<span>318</span><i class="muted">381 · 813 · 831 제외</i>'},
      {title:'처음과 끝을 모두 세어요',speech:'183쪽과 318쪽도 뜯은 페이지에 포함되므로 318−183 뒤에 1을 더합니다.',deep:'예를 들어 3쪽부터 5쪽은 5−3=2가 아니라 3·4·5의 3쪽이므로 +1이 필요합니다.',visual:'<span>318−183</span><i>+1</i>'},
      {title:'페이지 수를 계산해요',speech:'318−183+1=136이므로 뜯어낸 페이지는 136페이지입니다.',visual:'<span>318−183+1</span><i>=</i><span><b>136페이지</b></span>'}
    ]},
    8:{title:'같은 나머지를 갖는 수의 합',page:'002.jpg',view:'600 810 550 300',summary:'10부터 80까지 7로 나누어 나머지가 4인 수를 7씩 늘려 찾고 더합니다.',answer:'425',answerLabel:'조건에 맞는 수의 합',steps:[
      {title:'가장 작은 수부터 찾아요',speech:'10 이상에서 7로 나누어 나머지가 4인 가장 작은 수는 11입니다.',deep:'7×1+4=11입니다. 같은 나머지를 가진 다음 수는 7을 더하면 됩니다.',visual:'<span>7×1+4</span><i>=</i><span><b>11</b></span>'},
      {title:'7씩 늘려 끝까지 적어요',speech:'11, 18, 25, 32, 39, 46, 53, 60, 67, 74까지이고 다음 81은 범위를 넘습니다.',deep:'처음 11부터 마지막 74까지 모두 10개입니다.',visual:'<span>11 · 18 · 25 · … · 67 · 74</span><i>10개</i>'},
      {title:'처음과 끝을 짝지어요',speech:'처음과 끝의 합은 11+74=85이고, 같은 합이 되는 짝은 5쌍입니다.',deep:'10개를 양 끝에서 짝지으면 (11,74), (18,67)처럼 85가 되는 쌍이 5개입니다.',visual:'<span>85</span><i>×</i><span>5쌍</span>'},
      {title:'모든 수의 합을 구해요',speech:'85×5=425이므로 조건에 맞는 수의 합은 425입니다.',visual:'<span>85×5</span><i>=</i><span><b>425</b></span>'}
    ]},
    9:{title:'이동 경로를 거꾸로 돌아가기',page:'003.jpg',view:'45 120 530 570',summary:'동서와 남북 이동을 각각 상쇄하여 현재 위치에서 출발점까지 필요한 두 거리를 더합니다.',answer:'40',answerLabel:'□+△',steps:[
      {title:'동쪽과 서쪽을 서로 빼요',speech:'동쪽 41m에서 서쪽 17m를 빼면 출발점보다 동쪽으로 24m 와 있습니다.',deep:'출발점으로 돌아가려면 반대 방향인 서쪽으로 24m 가야 합니다.',visual:'<span>동 41</span><i>−</i><span>서 17</span><i>=</i><span><b>동 24</b></span>'},
      {title:'북쪽과 남쪽도 서로 빼요',speech:'북쪽 32m에서 남쪽 16m를 빼면 출발점보다 북쪽으로 16m 와 있습니다.',deep:'출발점으로 돌아가려면 반대 방향인 남쪽으로 16m 가야 합니다.',visual:'<span>북 32</span><i>−</i><span>남 16</span><i>=</i><span><b>북 16</b></span>'},
      {title:'돌아갈 두 거리를 식으로 놓아요',speech:'□는 서쪽으로 갈 24m, △는 남쪽으로 갈 16m입니다. 문제는 두 수의 합을 묻습니다.',deep:'방향을 묻는 것이 아니라 □+△의 값을 묻는지 마지막 문장을 다시 확인합니다.',visual:'<span>□=24</span><i>+</i><span>△=16</span>'},
      {title:'두 거리를 더해요',speech:'24+16=40이므로 □+△는 40입니다.',visual:'<span>24+16</span><i>=</i><span><b>40</b></span>'}
    ]},
    10:{title:'다섯 수 중 가장 작은 수',page:'003.jpg',view:'45 790 530 520',summary:'다섯 수의 합을 구하고 다른 네 수를 가장 크게 잡아 가장 작은 수의 최솟값을 찾습니다.',answer:'50',answerLabel:'가장 작은 수',steps:[
      {title:'나눗셈을 다섯 수의 합으로 바꿔요',speech:'다섯 수를 더해 5로 나눈 몫이 88이므로 다섯 수의 합은 5×88입니다.',deep:'나누어떨어졌으므로 나머지는 0이고 전체 합은 440입니다.',visual:'<span>5</span><i>×</i><span>88</span><i>=</i><span><b>440</b></span>'},
      {title:'가장 작은 수를 작게 만들어 봐요',speech:'한 수를 가장 작게 하려면 나머지 네 수를 99, 98, 97, 96으로 가장 크게 잡아야 합니다.',deep:'1부터 99까지 적힌 카드는 각각 한 장이므로 같은 수를 두 번 쓸 수 없습니다.',visual:'<span>99</span><span>98</span><span>97</span><span>96</span>'},
      {title:'큰 네 수를 전체에서 빼요',speech:'네 수의 합은 390입니다. 남은 한 수는 440−390으로 구합니다.',deep:'49 이하를 고르면 다른 네 수를 최대한 크게 해도 합이 439 이하라 440을 만들 수 없습니다.',visual:'<span>440</span><i>−</i><span>390</span>'},
      {title:'가장 작은 수를 확인해요',speech:'440−390=50이므로 가장 작은 수의 최솟값은 50입니다.',visual:'<span>440−390</span><i>=</i><span><b>50</b></span>'}
    ]},
    ...
    21:{title:'연속한 세 짝수의 곱',page:'006.jpg',view:'45 120 530 400',summary:'여섯 자리 수의 보이는 앞자리와 끝자리를 함께 만족하는 연속 짝수를 좁혀 갑니다.',answer:'62',answerLabel:'가장 작은 짝수',steps:[
      ...
      {title:'가장 작은 수의 시작 범위를 찾아요',speech:'56×58×60=194880은 20만보다 작습니다. 그보다 작은 세 짝수의 곱도 더 작으므로 가장 작은 수는 적어도 58입니다.',deep:'곱이 2로 시작하는 여섯 자리 수이려면 20만 이상이어야 합니다. 따라서 56 이하는 후보에서 빠지고 58부터 확인하면 됩니다.',visual:'<span>56×58×60</span><i>=</i><span>194880 &lt; 200000</span>'},
      {title:'58부터 차례대로 확인해요',speech:'58×60×62=215760, 60×62×64=238080은 끝이 8이 아닙니다. 62×64×66=261888은 두 조건을 모두 만족합니다.',deep:'후보를 건너뛰지 않고 58, 60, 62 순서로 확인했습니다. 261888은 2로 시작하고 8로 끝나는 여섯 자리 수입니다.',visual:'<span>58 → 끝 0</span><span>60 → 끝 0</span><span><b>62 → 끝 8</b></span>'},
      {title:'가장 작은 수를 골라요',speech:'조건을 처음 만족한 연속 짝수는 62, 64, 66이므로 가장 작은 수는 62입니다.',visual:'<span><b>62</b></span><span>64</span><span>66</span>'}
    ]},
    22:{title:'두 학년을 제외한 학생 수',page:'006.jpg',view:'45 780 530 460',summary:'4학년·6학년이 아닌 나머지 학년 전체를 한 묶음으로 놓고, 세 식을 더해 전체 학생 수를 찾습니다.',answer:'75명',answerLabel:'4학년과 6학년을 제외한 학생',steps:[
      {title:'나머지 학년 전체를 R이라 놓아요',speech:'4학년과 6학년이 아닌 모든 학생을 한 묶음 R이라 할게요. 6학년이 아닌 180명은 4학년+R입니다.',deep:'R은 특정 한 학년이 아니라 4학년과 6학년을 제외한 나머지 학년 학생 전부입니다. 원문에 없는 학년 수를 정하지 않아요.',visual:'<span>4학년+R<br><b>180</b></span><span>6학년+R<br><b>120</b></span><span>4학년+6학년<br><b>150</b></span>'},
      {title:'세 식을 더하면 전체의 두 배예요',speech:'180+120+150=450입니다. 4학년, 6학년, R이 각각 두 번씩 들어 있으니 전체는 450÷2입니다.',deep:'(4+R)+(6+R)+(4+6)=2×(4+6+R)입니다.',visual:'<span>450</span><i>÷2</i><span><b>전체 225</b></span>'},
      {title:'문제가 묻는 학생을 확인해요',speech:'4학년과 6학년을 제외하면 R만 남습니다. 4학년과 6학년의 합은 이미 150명이라고 주어졌어요.',deep:'각 학년의 수를 따로 구할 필요 없이 전체 225명에서 두 학년의 합 150명을 빼면 됩니다.',visual:'<span>전체 225</span><i>−</i><span>4학년+6학년 150</span>'},
      {title:'나머지 학년 전체를 구해요',speech:'225−150=75이므로 4학년과 6학년을 제외한 학생은 모두 75명입니다.',visual:'<span>225−150</span><i>=</i><span><b>75명</b></span>'}
    ]},
    ...
    28:{title:'삼각 수 배열의 이웃 합',page:'007.jpg',view:'600 700 550 600',summary:'원 안의 수를 제외하고 변이 맞닿은 여섯 칸을 원 안의 수와의 차로 나타냅니다.',answer:'30',answerLabel:'원 안의 수',steps:[
      ...
      {title:'맞은편 칸끼리 세 쌍으로 묶어요',speech:'보기의 9를 볼까요? 왼쪽·오른쪽은 8+10=18, 대각선 맞은편은 5+14=19와 6+13=19입니다.',deep:'원 안의 수가 ○이면 세 쌍의 합은 차례로 2○, 2○+1, 2○+1이 됩니다. 줄 번호를 몰라도 같은 묶음 규칙을 쓸 수 있어요.',visual:'<span>8+10<br><b>2×9</b></span><span>5+14<br><b>2×9+1</b></span><span>6+13<br><b>2×9+1</b></span>'},
      {title:'세 쌍의 합을 정리해요',speech:'2○+(2○+1)+(2○+1)=6○+2입니다. 여섯 이웃의 합이 182이므로 6○+2=182예요.',deep:'원 안의 ○ 자체는 더하지 않았습니다. 변이 맞닿은 여섯 칸을 맞은편끼리 묶어 더한 식입니다.',visual:'<span>2○</span><i>+</i><span>2○+1</span><i>+</i><span>2○+1</span><i>=</i><span><b>6○+2</b></span>'},
      {title:'○를 구해 배열에서 확인해요',speech:'6○=180이므로 ○=30입니다. 30은 여덟째 줄에 있고, 이웃 22, 23, 29, 31, 38, 39의 합은 182입니다.',visual:'<span>22+23+29+31+38+39</span><i>=</i><span><b>182</b></span>'}
    ]},
