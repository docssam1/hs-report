...
function pngInfo(name) {
  ...
  check(colorType === 6, `${name}: RGBA alpha`);
  return { file, width, height };
}

pngInfo('gpt-puppy-side-v1.png');
pngInfo('gpt-overlap-digits-no6-v1.png');
pngInfo('gpt-overlap-digits-no2-v1.png');
...
check(digitFunction.includes('gpt-overlap-digits-no6-v1.png') && digitFunction.includes('gpt-overlap-digits-no2-v1.png'), 'both digit variants consume GPT raster assets');
...
check(round1Html.includes('gpt-overlap-digits-no6-v1.png'), 'round 1 q12 uses no-6 digit art');
check(round2Html.includes('gpt-overlap-digits-no2-v1.png'), 'round 2 q10 uses no-2 digit art');
