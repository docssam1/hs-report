...
(async()=>{
 ...
 try{
  ...
  assert.match(adminHTML,/<h3[^>]*>⑩ 모의고사 결과<\/h3>\s*<label[^>]*for="apply-final-round"[^>]*>백분율 반영 회차<\/label>\s*<select[^>]*id="apply-final-round"[^>]*>[\s\S]*?<option value="1" selected>파이널 1회<\/option>[\s\S]*?<option value="4">파이널 4회<\/option>[\s\S]*?<\/select>\s*<button[^>]*id="apply-final-percentiles"/,'admin requires an explicit Final1-Final4 picker before the one-round action');
  ...
  const roundPicker=adminHTML.match(/<select[^>]*id="apply-final-round"[^>]*>[\s\S]*?<\/select>/)[0];
  const applyButton=adminHTML.match(/<button[^>]*id="apply-final-percentiles"[^>]*>[^<]*<\/button>/)[0];
  await page.setContent('<h3>모의고사 결과</h3><label for="apply-final-round">백분율 반영 회차</label>'+roundPicker+applyButton+'<span id="mock-status"></span>');
  ...
  await page.locator('#apply-final-round').selectOption('2');
  page.once('dialog',d=>{assert.match(d.message(),/파이널 2회/);d.dismiss();});await page.locator('#apply-final-percentiles').click();assert.equal(requests.filter(r=>r.action==='apply-percentiles').length,appliedBefore);
  offline=false;await page.locator('#apply-final-round').selectOption('3');page.once('dialog',d=>{assert.match(d.message(),/파이널 3회/);d.accept();});await page.locator('#apply-final-percentiles').click();await page.locator('#mock-status').filter({hasText:'파이널 3회 백분율을 저장했습니다'}).waitFor();
  assert.equal(requests.filter(r=>r.action==='apply-percentiles').length,appliedBefore+1);
  assert.equal(requests.filter(r=>r.action==='apply-percentiles').at(-1).exam,'final3','only the selected Final3 reference is sent');
  offline=true;savedRead=false;await page.locator('#apply-final-round').selectOption('4');page.once('dialog',d=>{assert.match(d.message(),/파이널 4회/);d.accept();});await page.locator('#apply-final-percentiles').click();await page.locator('#mock-status').filter({hasText:'저장하지 못했습니다'}).waitFor();
  assert.equal(requests.filter(r=>r.action==='apply-percentiles').at(-1).exam,'final4','a failed retry still targets only the selected Final4 reference');
