...
check('4개 일반 연습형과 1개 실제 원본 구조 검토형을 구분하고 공개는 차단', () => {
  assert.equal(summary.linkedLegacyGenerators, 5);
  assert.equal(summary.verifiedPracticeGenerators, 4);
  assert.equal(summary.sourceLinkedReviewTypes, 1);
  ...
  assert.deepEqual(links.map((type) => type.generator.generatorId).sort(), [
    'cube', 'overlap-range-sum', 'path', 'remainder', 'tri'
  ]);
  ...
  ['overlap-range-sum'].forEach((generatorId) => {
