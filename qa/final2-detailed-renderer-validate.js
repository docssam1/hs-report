...
// Exercise the presentation contract independently of the release gate. The
// production resolver still stays closed until the second-pass fields are set.
const approved=read('final2-detailed-data.js')
  .replaceAll("'first-pass-complete'","'verified'")
  .replaceAll("'pending-independent-second-pass'","'eligible'")
  .replace("independentReviewStatus:'pending'","independentReviewStatus:'verified'");
vm.runInContext(approved,box);
...
const html=w.GFIELD_FINAL1_DETAILED_RENDERER.render({
  ...
  renderDiagram:solution=>solution.diagram==='top-projection'?w.GFIELD_FINAL2_SOLUTION_DIAGRAMS.render(solution.no):''
});
assert.match(html,/id="final2DetailedSolutions" data-detailed-round="2"/);
assert.match(html,/12문항 \/ 전체 30문항/);
assert.equal((html.match(/final1-detailed-card is-ready/g)||[]).length,12);
assert.equal((html.match(/final1-detailed-card is-pending/g)||[]).length,18);
assert.match(html,/id="printFinal2Solutions"/);
assert.equal((html.match(/class="final1-data-table"/g)||[]).length,4,'all four reviewed step tables render');
...
console.log('PASS Final2 renderer: 12 reviewed, 18 pending, numbered steps, tables, Q12 SVG, print control and no private evidence fields');
